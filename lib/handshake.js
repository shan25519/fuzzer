      ...
      if (!skip(ExtensionType.KEY_SHARE)) {
        if (variant === 'pqc') {
          ...
          extensions.push(buildExtension(ExtensionType.KEY_SHARE, buildPQCKeyShareExtension([
            ...
            { group: NamedGroup.SECP256R1_MLKEM768, keySize: 1249 },
...
// ML-KEM key sizes and SPKI header sizes (Node.js exports SPKI-wrapped keys)
const MLKEM_ALGORITHMS = {
  768:  { algo: 'ml-kem-768',  ekSize: 1184, spkiHeaderSize: 22 },
  1024: { algo: 'ml-kem-1024', ekSize: 1568, spkiHeaderSize: 22 },
};

// Hybrid groups: component ordering per draft-ietf-tls-ecdhe-mlkem
// X25519MLKEM768:     ML-KEM ek (1184) || X25519 pk (32)   — PQC first
// SecP256r1MLKEM768:  EC point (65) || ML-KEM ek (1184)    — EC first (FIPS ordering)
// SecP384r1MLKEM1024: EC point (97) || ML-KEM ek (1568)    — EC first (FIPS ordering)
const HYBRID_GROUPS = {
  [NamedGroup.X25519_MLKEM768]:     { classical: 'x25519',    classicalSize: 32, mlkem: 768,  pqcFirst: true },
  [NamedGroup.SECP256R1_MLKEM768]:  { classical: 'prime256v1', classicalSize: 65, mlkem: 768,  pqcFirst: false },
  [NamedGroup.SECP384R1_MLKEM1024]: { classical: 'secp384r1',  classicalSize: 97, mlkem: 1024, pqcFirst: false },
};

// Standalone ML-KEM groups
const STANDALONE_MLKEM = {
  [NamedGroup.MLKEM512]:  { algo: 'ml-kem-512',  ekSize: 800,  spkiHeaderSize: 22 },
  [NamedGroup.MLKEM768]:  { algo: 'ml-kem-768',  ekSize: 1184, spkiHeaderSize: 22 },
  [NamedGroup.MLKEM1024]: { algo: 'ml-kem-1024', ekSize: 1568, spkiHeaderSize: 22 },
};

// Check if Node.js supports ML-KEM key generation (requires Node.js 23+ with OpenSSL 3.5+)
let _mlkemSupported = null;
function mlkemSupported() {
  if (_mlkemSupported === null) {
    try {
      crypto.generateKeyPairSync('ml-kem-768');
      _mlkemSupported = true;
    } catch {
      _mlkemSupported = false;
    }
  }
  return _mlkemSupported;
}

/**
 * Generate a valid ML-KEM encapsulation key.
 * Uses Node.js crypto if available (23+), otherwise generates
 * random bytes with coefficients clamped to valid range (< q=3329).
 */
function generateMLKEMKey(mlkemParam) {
  const info = MLKEM_ALGORITHMS[mlkemParam];
  if (!info) throw new Error(`Unknown ML-KEM parameter: ${mlkemParam}`);

  if (mlkemSupported()) {
    const { publicKey } = crypto.generateKeyPairSync(info.algo);
    const spki = publicKey.export({ type: 'spki', format: 'der' });
    return spki.slice(info.spkiHeaderSize);
  }

  // Fallback: generate plausible ML-KEM encapsulation key bytes.
  // ML-KEM ek encodes polynomial coefficients as 12-bit values (packed),
  // each must be < q (3329). Generate random 12-bit values mod 3329.
  const numCoeffs = Math.ceil(info.ekSize * 8 / 12);
  const coeffs = [];
  for (let i = 0; i < numCoeffs; i++) {
    coeffs.push(crypto.randomInt(3329));
  }
  // Pack 12-bit coefficients into bytes (little-endian bitstream)
  const buf = Buffer.alloc(info.ekSize);
  let bitPos = 0;
  for (const c of coeffs) {
    const byteIdx = bitPos >> 3;
    const bitOff = bitPos & 7;
    if (byteIdx < buf.length) buf[byteIdx] |= (c << bitOff) & 0xff;
    if (byteIdx + 1 < buf.length) buf[byteIdx + 1] |= (c >> (8 - bitOff)) & 0xff;
    if (bitOff > 4 && byteIdx + 2 < buf.length) buf[byteIdx + 2] |= (c >> (16 - bitOff)) & 0xff;
    bitPos += 12;
  }
  return buf;
}
 ...
 * EC groups produce real ECDH public keys; hybrid groups produce
 * [ML-KEM ek || classical pk] per the IETF spec; standalone ML-KEM
 * groups produce valid encapsulation keys via Node.js crypto.
 */
function generateKeyShareData(group, keySize) {
  ...
  const hybrid = HYBRID_GROUPS[group];
  if (hybrid) {
    const pqcPart = generateMLKEMKey(hybrid.mlkem);
    let classicalPart;
    if (hybrid.classical === 'x25519') {
      classicalPart = crypto.randomBytes(32); // X25519 accepts any 32 bytes
    } else {
      const ecdh = crypto.createECDH(hybrid.classical);
      ecdh.generateKeys();
      classicalPart = ecdh.getPublicKey(null, 'uncompressed');
    }
    // Component ordering differs: X25519 hybrids are PQC-first, EC hybrids are EC-first
    return hybrid.pqcFirst
      ? Buffer.concat([pqcPart, classicalPart])
      : Buffer.concat([classicalPart, pqcPart]);
  }

  const standalone = STANDALONE_MLKEM[group];
  if (standalone) {
    const mlkemParam = standalone.ekSize === 1184 ? 768 : standalone.ekSize === 1568 ? 1024 : 512;
    return generateMLKEMKey(mlkemParam);
  }

  // X25519 or experimental groups — random bytes are valid
