...
class FuzzerClient {
  ...
  async runScenario(scenario) {
    ...
    if (scenario.useNodeTLS || scenario.useNodeH2) {
      const { UnifiedClient } = require('./unified-client');
      const uc = new UnifiedClient({ host: this.host, port: this.port, timeout: this.timeout, delay: this.delay, logger: this.logger });
