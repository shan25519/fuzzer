        ...
        switch (action.type) {
          ...
          case 'recv': {
            ...
            if (data && data.length > 0) {
              ...
              // If we received a Handshake Failure alert, the scenario didn't "pass" in terms of connectivity
              const { parseRecords } = require('./record');
              const { records } = parseRecords(data);
              if (records.some(r => r.type === 21 && r.payload[1] === 40)) { // ALERT + HANDSHAKE_FAILURE
                status = 'DROPPED';
              }
  ...
  _waitForData(socket, timeout, isClosedFn) {
    return new Promise((resolve) => {
      ...
      const onData = (data) => {
        ...
        // Reset timer on each chunk — wait a bit for more
        clearTimeout(timer);
      ...
      const checkClosed = setInterval(() => {
        if (isClosedFn() || socket.destroyed) {
          ...
          setTimeout(done, 100); // small delay to collect final data
        }
      }, 50);
