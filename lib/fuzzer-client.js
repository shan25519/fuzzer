...
class FuzzerClient {
  ...
  /**
   * Run health probe — TCP connect only (HTTPS probe disabled as it almost
   * always fails after fuzz scenarios since the TLS state is disrupted)
   */
  async _runHealthProbes(host, port) {
    const tcp = await this._checkTCPAlive(host, port);
    return { tcp, https: tcp };
  }
