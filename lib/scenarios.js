...
const { FW_TLS_SCENARIOS, FW_SRV_SCENARIOS } = require('./firewall-scenarios');
const { SB_TLS_SCENARIOS } = require('./sandbox-scenarios');
const { APP_SCENARIOS, APP_CATEGORIES, APP_CATEGORY_SEVERITY } = require('./app-protocol-scenarios');
...
const CATEGORY_SEVERITY = {
  ...
  FV: 'info',    // Functional validation — full handshake + real HTTP traffic baseline
  FW: 'high',    // Firewall detection — known malicious payloads that should be blocked
  SB: 'high',    // Sandbox detection — malicious JS/dynamic content in server responses
  SRV: 'high',   // TLS server-side fuzzing — malicious server behavior against client
  SCAN: 'info',  // Connectivity scanning — non-fuzzing
  PAN: 'info',   // PAN-OS URL Category SNI Probes
  'PAN-PQC': 'info', // PAN-OS PQC + SNI Evasion Probes
  ...
  AM: 'info',     // HTTP/2 Functional Validation — full handshake + real HTTP/2 traffic baseline
  AN: 'high',     // HTTP/2 Firewall Detection — malicious payloads that should be blocked
  AO: 'high',     // HTTP/2 Sandbox Detection — malicious JS/dynamic content in responses
  ...
  H2S: 'high',    // HTTP/2 Server-Side Fuzzing — malicious server behavior against client
  // QUIC server-side category
  QS: 'high',     // QUIC Server-Side Fuzzing — malicious server behavior against client
...
const CATEGORY_DEFAULT_DISABLED = new Set(['Z', 'SRV', 'RA', 'RB', 'RC', 'RD', 'RE', 'RF', 'RG']);
...
const SCENARIOS = [
  ...
  // These implement a proper TLS 1.2 handshake state machine with validation.
  // Each recv is followed by a validate action that checks the received TLS
  // records contain the expected handshake message types. If validation fails,
  // a fatal UNEXPECTED_MESSAGE alert is sent and the handshake is aborted.
  {
    name: 'fv-tls-well-behaved',
    category: 'FV',
    description: 'Compliant TLS client handshake + HTTP GET request (functional validation)',
    side: 'client',
    useNodeTLS: true,
    clientHandler: async (socket, host, logger, pcap) => {
      const req = `GET / HTTP/1.1\r\nHost: ${host}\r\nConnection: close\r\n\r\n`;
      socket.write(req);
      if (pcap) pcap.writeTLSData(Buffer.from(req), 'sent');
      ...
      return { status: 'PASSED', response: `Handshake successful, received ${data.length} bytes` };
    },
    expected: 'PASSED',
  },
  ...
  // ===== Category FV: Functional Validation (TLS) =====
  // Full TLS handshake via Node.js API + valid HTTP GET/POST requests.
  // These verify that the target correctly handles normal traffic after a successful handshake.
  {
    name: 'fv-tls-get',
    category: 'FV',
    description: 'Functional: full TLS handshake + HTTP GET request, validate 200 OK response',
    side: 'client',
    useNodeTLS: true,
    clientHandler: async (socket, host, logger, pcap) => {
      ...
      logger.info(`[fv] GET response: ${data.length} bytes`);
      if (resp.includes('200')) return { status: 'PASSED', response: `HTTP 200 OK (${data.length} bytes)` };
      if (data.length > 0) return { status: 'PASSED', response: `Response: ${data.length} bytes` };
      return { status: 'DROPPED', response: 'No response to GET request' };
    },
    expected: 'PASSED',
  },
  {
    name: 'fv-tls-post-small',
    category: 'FV',
    description: 'Functional: full TLS handshake + HTTP POST with 256-byte body, validate echo',
    side: 'client',
    useNodeTLS: true,
    clientHandler: async (socket, host, logger) => {
      ...
      logger.info(`[fv] POST response: ${data.length} bytes`);
      if (data.length > 0) {
        const resp = data.toString();
        const echoed = resp.includes('200') && data.includes(body);
        return { status: 'PASSED', response: echoed ? `POST echo verified (${body.length} bytes)` : `Response: ${data.length} bytes` };
      }
      return { status: 'DROPPED', response: 'No response to POST request' };
    },
    expected: 'PASSED',
  },
  {
    name: 'fv-tls-post-large',
    category: 'FV',
    description: 'Functional: full TLS handshake + HTTP POST with 64KB body, validate echo',
    side: 'client',
    useNodeTLS: true,
    clientHandler: async (socket, host, logger) => {
      const body = crypto.randomBytes(65536);
      const req = `POST / HTTP/1.1\r\nHost: ${host}\r\nContent-Length: ${body.length}\r\nConnection: close\r\n\r\n`;
      socket.write(req);
      socket.write(body);
      const data = await new Promise((resolve) => {
        ...
        socket.on('data', (d) => { buf = Buffer.concat([buf, d]); });
        socket.on('end', () => resolve(buf));
        setTimeout(() => resolve(buf), 10000);
      });
      logger.info(`[fv] POST 64KB response: ${data.length} bytes`);
      if (data.length > 0) {
        const resp = data.toString('utf8', 0, Math.min(data.length, 500));
        const echoed = resp.includes('200') && data.includes(body);
        return { status: 'PASSED', response: echoed ? `POST echo verified (${body.length} bytes)` : `Response: ${data.length} bytes` };
      }
      return { status: 'DROPPED', response: 'No response to POST 64KB' };
    },
    expected: 'PASSED',
  },
  {
    name: 'fv-tls-sequential-requests',
    category: 'FV',
    description: 'Functional: full TLS handshake + 10 sequential HTTP GET requests on same connection',
    side: 'client',
    useNodeTLS: true,
    clientHandler: async (socket, host, logger) => {
      let successCount = 0;
      ...
      for (let i = 0; i < totalRequests; i++) {
        ...
        const data = await new Promise((resolve) => {
          ...
          setTimeout(() => { socket.removeListener('data', onData); resolve(buf); }, 3000);
        });
        if (data.length > 0 && data.toString().includes('200')) successCount++;
      }

      logger.info(`[fv] Sequential: ${successCount}/${totalRequests} responses received`);
      if (successCount === totalRequests) return { status: 'PASSED', response: `${successCount}/${totalRequests} sequential GET requests OK` };
      if (successCount > 0) return { status: 'PASSED', response: `${successCount}/${totalRequests} responses (partial keep-alive)` };
      return { status: 'DROPPED', response: 'No responses to sequential requests' };
    },
    expected: 'PASSED',
  },
  {
    name: 'fv-tls-head-request',
    category: 'FV',
    description: 'Functional: full TLS handshake + HTTP HEAD request, validate empty body with headers',
    side: 'client',
    useNodeTLS: true,
    clientHandler: async (socket, host, logger) => {
      ...
      const resp = data.toString();
      logger.info(`[fv] HEAD response: ${data.length} bytes`);
      if (resp.includes('200')) return { status: 'PASSED', response: `HEAD 200 OK (${data.length} bytes header)` };
      if (data.length > 0) return { status: 'PASSED', response: `Response: ${data.length} bytes` };
      return { status: 'DROPPED', response: 'No response to HEAD request' };
    },
    expected: 'PASSED',
  },
  {
    name: 'fv-tls-post-then-get',
    category: 'FV',
    description: 'Functional: full TLS handshake + POST then GET on same connection, validate both responses',
    side: 'client',
    useNodeTLS: true,
    clientHandler: async (socket, host, logger) => {
      const readResponse = () => new Promise((resolve) => {
        ...
        socket.on('data', onData);
        setTimeout(() => { socket.removeListener('data', onData); resolve(buf); }, 5000);
      });

      // POST request
      const body = Buffer.from('Hello, server!');
      socket.write(`POST / HTTP/1.1\r\nHost: ${host}\r\nContent-Length: ${body.length}\r\nConnection: keep-alive\r\n\r\n`);
      socket.write(body);
      const postResp = await readResponse();

      // GET request
      socket.write(`GET / HTTP/1.1\r\nHost: ${host}\r\nConnection: close\r\n\r\n`);
      const getResp = await readResponse();

      const postOk = postResp.length > 0 && postResp.toString().includes('200');
      const getOk = getResp.length > 0 && getResp.toString().includes('200');
      logger.info(`[fv] POST+GET: POST=${postResp.length}B GET=${getResp.length}B`);

      if (postOk && getOk) return { status: 'PASSED', response: 'POST + GET both 200 OK' };
      if (postOk || getOk) return { status: 'PASSED', response: `POST=${postOk ? 'OK' : 'fail'} GET=${getOk ? 'OK' : 'fail'}` };
      return { status: 'DROPPED', response: 'No responses to POST+GET sequence' };
    },
    expected: 'PASSED',
  },

  // ╔══════════════════════════════════════════════════════════════════════╗
  // ║              SERVER-SIDE SCENARIOS (side: server)                   ║
  // ║  These scenarios run a fuzzed TLS server; a well-behaved client     ║
  // ║  connects and should reject the malformed server responses.         ║
  // ╚══════════════════════════════════════════════════════════════════════╝
...
function generateCHSizeVariants() {
  ...
  const serverScenarios = SCENARIOS.filter(s => s.side !== 'client');
  ...
  for (const sc of clientScenarios) {
    ...
    variants.push({
      ...
      actions: typeof sc.actions === 'function' ? (opts) => {
    ...
    // Post-Quantum big ClientHello variant — forces multi-segment/multi-record handshake
    variants.push({
      ...sc,
      name: sc.name + '-pqc-ch',
...
// Push PAN TLS scenarios
SCENARIOS.push(...getPanSniScenarios('tls'));

// Push PAN-PQC TLS scenarios
SCENARIOS.push(...getPanPqcScenarios('tls'));

// Push Firewall Detection TLS scenarios
SCENARIOS.push(...FW_TLS_SCENARIOS);

// Push Firewall Server-side Detection scenarios (SRV category)
SCENARIOS.push(...FW_SRV_SCENARIOS.filter(s => s.category === 'SRV'));

// Push Sandbox Detection TLS scenarios
SCENARIOS.push(...SB_TLS_SCENARIOS);

// Push Application Protocol Vulnerability Scenarios
SCENARIOS.push(...APP_SCENARIOS);
...
function getClientScenarios() {
  ...
  return SCENARIOS.filter(s => s.side === 'client').concat(scans.filter(s => s.side === 'client'));
}
