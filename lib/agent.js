...
function startAgent(role, opts = {}) {
  ...
  async function runScenarios() {
    ...
    if (role === 'client') {
      const workers = parseInt(config.workers) || 1;
      let host = config.host || 'localhost';
      let port = parseInt(config.port) || 443;

      if (workers > 1) {
        broadcastEvent({ type: 'logger', event: { type: 'info', ts: new Date().toISOString(), message: `Starting distributed run with ${workers} worker threads...` } });
        ...
        const runNext = () => {
          ...
          worker.on('message', (msg) => {
            if (msg.type === 'logger') {
              ...
              worker.send({ type: 'ack' }); // Safe to exit now
          ...
          worker.on('exit', () => {
            active--;
            completed++;
            broadcastEvent({ type: 'progress', current: completed, total });
      ...
      for (let i = 0; i < scenarios.length; i++) {
        ...
        try {
          ...
          // Skip well-behaved counterpart results — they are internal helpers, not actual tests
          if (result.scenario && result.scenario.startsWith('well-behaved-')) continue;
          state.results.push(result);
          broadcastEvent({ type: 'result', result });
        } catch (err) {
          if (scenarios[i].name && scenarios[i].name.startsWith('well-behaved-')) continue;
          const errResult = {
            ...
            compliance: null,
            finding: { grade: 'FAIL', severity: 'high', reason: `Scenario crashed: ${err.message}` },
            hostDown: false,
            probe: null,
          };
          state.results.push(errResult);
          broadcastEvent({ type: 'result', result: errResult });
        }
        await sleep(300);
      }
      ...
      // Server role — support parallel workers or persistent well-behaved mode
      const workers = parseInt(config.workers) || 1;
      ...
      // Optimization: If all scenarios are well-behaved, just start ONE persistent server.
      // This is the standard case for distributed client-side fuzzing.
      const allWellBehaved = scenarios.every(s => s.name && s.name.startsWith('well-behaved-'));
      
      if (allWellBehaved) {
        broadcastEvent({ type: 'logger', event: { type: 'info', ts: new Date().toISOString(), message: `High-concurrency detected: Starting persistent well-behaved server on port ${port}...` } });
        
        const { WellBehavedServer } = require('./well-behaved-server');
        const server = new WellBehavedServer({ port, hostname, logger });
        state.fuzzer = server;
        
        if (protocol === 'h2') await server.startH2();
        else if (protocol === 'quic') await server.startQuic();
        else if (protocol === 'raw-tcp') await server.startTCP();
        else await server.startTLS();
      ...
      if (workers > 1) {
        broadcastEvent({ type: 'logger', event: { type: 'info', ts: new Date().toISOString(), message: `Starting distributed server run with ${workers} worker threads...` } });
        ...
        const runNext = () => {
          ...
          active++;
          const workerConfig = { ...config, role };
          const worker = fork(path.join(__dirname, 'agent-worker.js'), [ JSON.stringify(workerConfig), scenarioName ]);
          worker.on('message', (msg) => {
            if (msg.type === 'logger') broadcastEvent({ type: 'logger', event: msg.event });
            else if (msg.type === 'result') {
              if (!(msg.result.scenario && msg.result.scenario.startsWith('well-behaved-'))) {
                state.results.push(msg.result);
                broadcastEvent({ type: 'result', result: msg.result });
              }
              worker.send({ type: 'ack' });
            }
          });
          worker.on('exit', () => {
            active--; completed++;
        ...
        for (let i = 0; i < Math.min(workers, total); i++) runNext();
        return;
      }
