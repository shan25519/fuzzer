...
function startAgent(role, opts = {}) {
            ...
            try {
              ...
              worker.on('message', (msg) => {
                try {
                  ...
                  if (msg.type === 'result' && worker.connected) worker.send({ type: 'ack' });
                }
              });
  ...
  httpServer.listen(controlPort, '::', () => {
    ...
    console.log(`  Control API   http://[::]:${controlPort}`);
