...
class Logger {
  ...
  healthProbe(host, port, probe) {
    ...
    this._emit(event);
    if (!this.json) {
      // All probes are now ping-based — use tcp field which holds the ping result
  ...
  summary(results, report = null) {
    ...
    if (probed > 0) {
      console.log(`  ${COLORS.blue}Health Probes: ${probed}${COLORS.reset}  ${COLORS.green}Ping OK: ${pingOk}/${probed}${COLORS.reset}`);
    }
