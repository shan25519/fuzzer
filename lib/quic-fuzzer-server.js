...
class QuicFuzzerServer {
  ...
  async start() {
    ...
    const setupSocket = () => {
      const isLocal = this.hostname === 'localhost' || this.hostname === '127.0.0.1' || this.hostname === '::1';
      const type = (this.hostname.includes(':') || isLocal) ? 'udp6' : 'udp4';
      const sock = dgram.createSocket({ type, ipv6Only: false });
      sock.on('error', (err) => {
    ...
    await new Promise((resolve, reject) => {
      const bindWithRetry = (sock, retriesLeft) => {
        ...
        sock.bind(this.port, bindAddr, () => {
          this.logger.info(`QUIC server listening on ${bindAddr}:${this.port} (UDP)`);
          resolve();
        });
