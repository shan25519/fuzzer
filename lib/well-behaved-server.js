...
class WellBehavedServer {
  ...
  async startQuic() {
    ...
    await this._quicServer.listen(listenPort, this.hostname);
    this._actualPort = listenPort;
    if (this.logger) this.logger.info(`[local-server] HTTP/3 server listening on UDP ${this.hostname}:${this._actualPort} (quiche)`);
  }
  ...
  async _startQuicRaw() {
    ...
    const type = this.hostname.includes(':') ? 'udp6' : 'udp4';
    this._quicSocket = dgram.createSocket(type);
    ...
    await new Promise((resolve, reject) => {
      this._quicSocket.bind(listenPort, this.hostname, () => {
        if (this.logger) this.logger.info(`[local-server] Node QUIC server listening on UDP ${this.hostname}:${this._actualPort} (raw fallback)`);
        resolve();
      });
