...
class WellBehavedServer {
  ...
  async startQuic() {
    ...
    await this._quicServer.listen(listenPort, '0.0.0.0');
    this._actualPort = listenPort;
    if (this.logger) this.logger.info(`[local-server] HTTP/3 server listening on UDP port ${this._actualPort} (quiche)`);
  }
  ...
  async _startQuicRaw() {
    ...
    await new Promise((resolve, reject) => {
      this._quicSocket.bind(listenPort, '0.0.0.0', () => {
        if (this.logger) this.logger.info(`[local-server] Node QUIC server listening on UDP port ${this._actualPort} (raw fallback)`);
        resolve();
      });
