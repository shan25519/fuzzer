...
class UnifiedServer {
  ...
  _runTLSScenario(scenario) {
    return new Promise((resolve) => {
      ...
      const resolveOnce = (result) => {
        if (resolved) return;
        resolved = true;
        // Wait for the server to fully close before resolving
        // so the port is available for the next scenario
        if (this.tlsServer) {
          const srv = this.tlsServer;
          this.tlsServer = null;
          srv.close(() => resolve(result));
          // Safety: if close callback doesn't fire within 500ms, resolve anyway
      ...
      this.tlsServer = net.createServer({ allowHalfOpen: true }, async (socket) => {
        ...
        // Finalize response: if all actions completed (PASSED) but lastResponse
        // just describes received data (e.g. ClientHello), replace with outcome
        if (status === 'PASSED' && lastResponse && /^ClientHello\(/.test(lastResponse)) {
          lastResponse = 'Handshake completed';
        }

        // Response-aware status: alert from client = tls-alert-client
  ...
  _runNodeTLSServer(scenario) {
    return new Promise((resolve) => {
      ...
      server.on('tlsClientError', (err, socket) => {
        ...
        // Extract alert info from the error
  ...
  async startH2() {
    ...
    // Default handler: when a scenario is active, queue the stream so the
    // scenario handler can pick it up. When idle (passive mode), respond 200.
    this._h2PendingStreams = [];
    this.h2Server.on('stream', (stream, headers) => {
      if (this._h2ScenarioActive) return; // scenario's own once('stream') will fire
      // Queue streams that arrive between scenarios (client connected early)
  ...
  async _runH2Scenario(scenario) {
    ...
    return new Promise((resolve) => {
      ...
      const onStream = (stream) => { handleStream(stream); };
