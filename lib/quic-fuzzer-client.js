...
class QuicFuzzerClient {
   ...
   */
  _ensureHealthSocket() {
    if (this._healthSocket) return this._healthSocket;
    this._healthSocket = dgram.createSocket('udp4');
    this._healthSocket.on('error', () => {
      try { this._healthSocket.close(); } catch (_) {}
      this._healthSocket = null;
    });
    // Unref so it doesn't prevent process exit
    this._healthSocket.unref();
    return this._healthSocket;
  }

  _buildHealthProbePacket() {
  ...
  _checkUDPAlive(host, port, timeout = 2000) {
    ...
    return new Promise((resolve) => {
      const socket = this._ensureHealthSocket();
      if (!socket) return resolve({ alive: false, error: 'no socket' });
      ...
      const onMessage = () => done({ alive: true, latency: Date.now() - start });
      socket.on('message', onMessage);

      const packet = this._buildHealthProbePacket();
      socket.send(packet, port, host, (err) => {
        if (err) done({ alive: false, error: err.code || err.message });
      });

      const timer = setTimeout(() => done({ alive: false, error: 'timeout' }), timeout);
    });
  }
