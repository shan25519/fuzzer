...
function computeExpected(scenario) {
  ...
  if (EXPLICIT_PASSED[scenario.name] || scenario.category === 'SCAN' || scenario.category === 'Z') {
    return {
      expected: 'PASSED',
      reason: scenario.category === 'SCAN' ? 'Baseline connectivity scan' : (EXPLICIT_PASSED[scenario.name] || 'Well-behaved counterpart'),
    };
  }
