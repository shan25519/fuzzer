...
class UnifiedClient {
    ...
    for (const r of records) {
      if (r.type === ContentType.HANDSHAKE && r.payload.length >= 1) {
        ...
        if (hsType === HandshakeType.SERVER_HELLO && r.payload.length >= 40) {
          ...
          return `ServerHello(${VersionName[bodyVersion] || '0x' + bodyVersion.toString(16)})`;
        }
  ...
  /**
   * Simple TCP connect health probe — checks if the target port is accepting
   * connections. No TLS or HTTP overhead; just verifies the host is up.
   */
  async _runHealthProbes(host, port) {
    ...
    return new Promise((resolve) => {
      const socket = net.connect({ host, port }, () => {
        ...
        socket.destroy();
        resolve({ tcp: { alive: true, latency }, https: { alive: true, latency } });
      });
      socket.setTimeout(3000, () => {
        socket.destroy();
        resolve({ tcp: { alive: false, error: 'timeout' }, https: { alive: false, error: 'timeout' } });
      });
      socket.on('error', (err) => {
        resolve({ tcp: { alive: false, error: err.message }, https: { alive: false, error: err.message } });
      });
    });
  }
