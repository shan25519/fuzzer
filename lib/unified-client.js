  ...
  async _runNodeTLSClient(scenario) {
    ...
    try {
      ...
      if (this.pcap) {
        ...
        socket.on('data', (d) => this.pcap.writeTLSData(d, 'received'));
        // Hook write to capture sent data
  ...
  async _runNodeH2Client(scenario) {
    ...
    try {
      ...
      if (this.pcap) {
        this.pcap.writeTCPHandshake();
        if (session.socket) {
          session.socket.on('data', (d) => this.pcap.writeTLSData(d, 'received'));
          const origWrite = session.socket.write.bind(session.socket);
          session.socket.write = (data, cb) => {
            this.pcap.writeTLSData(data, 'sent');
            return origWrite(data, cb);
          };
        }
