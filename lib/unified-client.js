...
class UnifiedClient {
  ...
  async runScenario(scenario) {
    ...
    if (isQuicScenario(scenario)) {
      // First, check if the user has installed a native quiche library.
      // If so, we route ALL QUIC scenarios (fuzzing and baseline) through the native state machine.
      let quicheLib = null;
      try {
        quicheLib = require('@currentspace/http3');
      } catch (e) {
        // Not installed, fall back to default behavior
      }
      ...
      // If no native library is present, use OpenSSL for baseline scenarios
    ...
    if (scenario.useNodeH2) return this._runNodeH2Client(scenario);
    
    // If scenario defines manual actions, always use the TLS/TCP fuzzer engine
    if (typeof scenario.actions === 'function' || Array.isArray(scenario.actions)) {
      return this._runTLSScenario(scenario);
    }
  ...
  async _runNodeTLSClient(scenario) {
    ...
    try {
      const socket = await this._connectNodeTLSWithRetry(scenario.nodeTlsOptions || {});

      this.logger.info(`[node-tls] TLS connected (${socket.getProtocol()})`);

      if (scenario.clientHandler) {
        // Scenario defines custom client logic
        const result = await scenario.clientHandler(socket, this.host, this.logger);
  ...
  async _runNodeH2Client(scenario) {
    ...
    try {
      const session = await this._connectNodeH2WithRetry(scenario.nodeTlsOptions || {});
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      try {
        return await new Promise((resolve, reject) => {
          const session = http2.connect(`https://${this.host}:${this.port}`, {
            rejectUnauthorized: false,
            ...options
          });
          session.on('connect', () => resolve(session));
          session.on('error', (err) => reject(err));
          setTimeout(() => reject(new Error('H2 connect timeout')), this.timeout);
        });
      } catch (err) {
